...
export function TextImages({ section }: { section: IResourceSection }) {
  const { id: resourceId } = useParams();

  const [images, setImages] = useState<IResourceImage[]>(section.images || []);
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const image = await fetcherAdmin(
        `/admin/resources/${resourceId}/content-section/${section.id}/text-image`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
...
function TextImage({
  ...
  const removeImageHandle = async (id: number) => {
    ...
    try {
      await fetcherAdmin(
        `/admin/resources/${resourceId}/content-section/${sectionId}/text-image/${id}`,
        {
          method: "DELETE",
          credentials: "include",
        },
