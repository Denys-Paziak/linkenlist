...
export function List() {
  ...
  const [showFavoritesOnly, setShowFavoritesOnly] =
    useQueryStateWithLocalStorage("/deals?favorites", {
      defaultValue: false,
  ...
  const [limit, setLimit] = useQueryStateWithLocalStorage("/deals?limit", {
    defaultValue: 16,
    parse: (v) => parseAsInteger.parse(v),
    sync: true,
  });
              ...
              {data?.[0].length === 0 && !isLoading && (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mt-2 text-sm">
                    {showFavoritesOnly
                      ? "Start adding deals to your favorites by clicking the star icon on any card."
