...
export class DealCronService {
	private readonly logger = new Logger(DealCronService.name)
	...
	async updateDealsStatistics() {
		this.logger.log('🔄 Recomputing deal stats (views + helpful + freshness)...')

		const sql = `
            WITH views_agg AS (
                SELECT
                    dv.entity_id AS deal_id,
            ...
            joined AS (
                SELECT
                    ...
                    p.last_published_at
                FROM pub p
                LEFT JOIN views_agg   v ON v.deal_id = p.id
                LEFT JOIN helpful_agg h ON h.deal_id = p.id
            ),
            mx AS (
                SELECT
		...
		await this.dealRepository.query(sql, [EDailyMetricType.DEAL_VIEW, EDailyMetricType.DEAL_HELPFUL, EDealStatus.PUBLISHED])

		this.logger.log('✅ Deal stats updated.')
	}
}
